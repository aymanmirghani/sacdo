...
import { AdminStackParamList, Payment } from '../../types';
import { getMemberPayments, recordPayment, getMembershipFees, getPaymentTypes } from '../../services/members';
import { useAuthStore } from '../../store/useAuthStore';

type Props = NativeStackScreenProps<AdminStackParamList, 'MemberPayments'>;

export default function MemberPaymentsScreen({ route }: Props) {
  ...
  async function load() {
    setLoading(true);
    try {
      const [p, f, pt] = await Promise.all([getMemberPayments(memberId), getMembershipFees(), getPaymentTypes()]);
      setPayments(p);
  ...
  async function handleRecord() {
    if (!form.amount || !form.paymentDate || !form.membershipType) {
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.empty}>No payments recorded for this member.</Text>
        }
        refreshing={loading}
        onRefresh={load}
      />

      <FAB icon="plus" label="Record Payment" style={styles.fab} onPress={openDialog} />
      ...
      <Portal>
        <Dialog visible={dialogVisible} onDismiss={() => setDialogVisible(false)}>
          ...
          <Dialog.Actions>
            ...
            <Button onPress={handleRecord} loading={saving} disabled={saving}>Save</Button>
          </Dialog.Actions>
        </Dialog>
      </Portal>
    </View>
